...
import { ArrowRight, Calendar, Clock, MapPin, Shield, UserPlus, Car } from 'lucide-react';
...
const JoinRide = () => {
  ...
  const handleJoin = async () => {
    ...
    if (ride.ladiesOnly && userGender !== 'Female') {
        showModal({
            title: "Access Denied",
            message: "⛔ This ride is marked as Ladies Only. For safety, male users cannot join.",
            type: "alert"
        });
        return;
    }
    ...
    showModal({
        ...
        onConfirm: async () => {
            ...
            navigate('/rides'); // Go to feed after joining
        }
    });
  };
  ...
  if (loading) return <div className="text-white text-center mt-20">Loading Ride Details...</div>;
  if (!ride) return null;
        ...
        >
            ...
            <div className="flex justify-between items-start mb-6">
                <div className="flex gap-2">
                    <span className="bg-white/10 text-white px-3 py-1 rounded-lg text-xs font-bold uppercase">{ride.vehicleType}</span>
                    {ride.ladiesOnly && (
            ...
            <div className="flex items-center gap-3 text-white font-bold text-xl mb-6">
                <span>{ride.from}</span> <ArrowRight className="text-gray-500"/> <span>{ride.to}</span>
            </div>
            ...
            {/* ACTION BUTTON */}
            {isHost ? (
                <button onClick={() => navigate('/rides')} className="w-full py-4 rounded-xl font-bold bg-white/10 text-white">You are the Host</button>
            ) : isPassenger ? (
                <button onClick={() => navigate('/rides')} className="w-full py-4 rounded-xl font-bold bg-green-500/20 text-green-400 border border-green-500/50">Already Joined</button>
            ) : seatsLeft <= 0 ? (
                <button disabled className="w-full py-4 rounded-xl font-bold bg-red-500/10 text-red-500 cursor-not-allowed">Ride Full</button>
            ) : (
                <button onClick={handleJoin} className="w-full py-4 rounded-xl font-bold bg-white text-black hover:bg-gray-200 transition flex items-center justify-center gap-2 shadow-lg">
